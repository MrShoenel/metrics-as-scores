...
def generate_parametric_fits(data_dict: dict[str, NDArray[Shape["*"], Float]], dist_transform: DistTransform, transform_values_dict: dict[str, float]) -> dict[str, DensityFunc]:
    ...
    from scipy.stats._continuous_distns import norminvgauss_gen, genhyperbolic_gen, geninvgauss_gen, invgauss_gen, studentized_range_gen
    from scipy.stats._discrete_distns import nhypergeom_gen, hypergeom_gen
    ignored_dists = list([rv.__name__ for rv in [norminvgauss_gen, genhyperbolic_gen, geninvgauss_gen, invgauss_gen, studentized_range_gen, nhypergeom_gen, hypergeom_gen]])
...
if __name__ == '__main__':
    ...
    for dist_transform in list(DistTransform):
        ...
        res = Parallel(n_jobs=min(len(MetricID), cpu_count()))(delayed(get_data_tuple)(dist, m, dist_transform) for m in tqdm(list(MetricID)))
        ...
        del res
        print(f'Having {len(list(data_dict.keys()))} pre-processed datasets.')

        result = generate_parametric_fits(data_dict=data_dict, dist_transform=dist_transform, transform_values_dict=transform_values_dict)
